...
SignalBrowserMouseHandling::Action SignalBrowserMouseHandling::getAction(QGraphicsSceneMouseEvent* e, PortingToQT4_::SignalBrowserModel::Mode mode)
{
        // hand scroll
    if (e->button() == Qt::MidButton  && e->modifiers() == Qt::NoModifier ||
        mode == PortingToQT4_::SignalBrowserModel::MODE_HAND && e->button() == Qt::LeftButton)
    {
        return HAND_SCROLL_ACTION;
    }

    // zoom window
    if (mode == PortingToQT4_::SignalBrowserModel::MODE_ZOOM && e->button() == Qt::LeftButton)
    {
        return ZOOM_WINDOW_ACTION;
    }
    ...
    if (e->button() == Qt::MidButton && e->modifiers().testFlag(Qt::ShiftModifier) ||
        mode == PortingToQT4_::SignalBrowserModel::MODE_SHIFT_SIGNAL && e->button() == Qt::LeftButton)
    {
        return SHIFT_CHANNEL_ACTION;
    }

    // new event
    if (e->button() == Qt::LeftButton && mode == PortingToQT4_::SignalBrowserModel::MODE_NEW)
    {
        return NEW_EVENT_ACTION;
    }
    ...
    // select or resize event
    if (e->button() == Qt::LeftButton && e->modifiers() == Qt::NoModifier &&
        mode == PortingToQT4_::SignalBrowserModel::MODE_POINTER)
    {
        return SELECT_RESIZE_EVENT_ACTION;
    }
