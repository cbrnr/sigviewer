...
void animateProperty(QObject* target,
    ...
    QObject* call_back_object,
    char const* call_back_slot) {
    QSettings settings;
...
QList<QSharedPointer<SignalEvent const> > getSelectedEvents(QSharedPointer<EventView> event_view) {
    ...
    foreach (EventID id, ids) events.append(event_manager->getEvent(id));
...
QList<EventID> getSelectedEventIDs(QSharedPointer<EventView> event_view) {
    ...
    if (event_view.isNull()) return event_ids;
...
EventType selectEventType(EventType preselected_type,
    ...
    for (std::set<EventType>::const_iterator it = types.begin(); it != types.end(); it++) {
        if (preselected_type == *it) current_item = event_type_list.size();

        QString event_name = event_manager->getNameOfEventType(*it);
        event_type_list
            .append(event_name + " " + QString("(%1)").arg(*it, 4, 16).replace(' ', '0'));
    }

    bool ok = false;
    QString res = QInputDialog::getItem(0,
    ...
    if (ok) new_type = res.right(5).left(4).toUShort(0, 16);
...
std::set<EventType> selectEventTypes(std::set<EventType> const& preselected_type,
    ...
    if (event_manager.isNull()) return selected_types;
...
ChannelID selectChannel(ChannelID preselected_channel_id,
    ...
    for (std::set<ChannelID>::iterator channel_iter = shown_channels.begin();
        ...
        ++channel_iter) {
        if (*channel_iter == preselected_channel_id) current_item = channel_list.size();

        channel_list.append(QString("(%1) ").arg(*channel_iter + 1)
                            + channel_manager.getChannelLabel(*channel_iter));
    }

    bool ok = false;
    QString res = QInputDialog::getItem(0,
        QObject::tr("Change Channel"),
...
std::set<ChannelID> selectChannels(ChannelManager const& channel_manager,
    ...
    if (!vis_model.isNull()) pre_selected_channels = vis_model->getShownChannels();

    bool empty_selection = (pre_selected_channels.size() == 0);

    for (const auto channel_id : channel_manager.getChannels()) {
        bool show_channel = (empty_selection || (pre_selected_channels.count(channel_id) > 0));
    ...
    if (channel_dialog.result() == QDialog::Rejected) return pre_selected_channels;

    std::set<ChannelID> selected_channels;
    for (const auto channel_id : channel_manager.getChannels()) {
        if (channel_dialog.isSelected(channel_id)) selected_channels.insert(channel_id);
    }
...
QString getFilePathFromSaveAsDialog(QString const& path,
    ...
    QString extension_selection = file_type_description + QString(" (%1)").arg(extensions);
    ...
    extension_selection += ";; *.* (*.*)";
    return QFileDialog::getSaveFileName(0, QObject::tr("Save"), path, extension_selection);
}
