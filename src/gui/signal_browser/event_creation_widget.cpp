...
EventCreationWidget::EventCreationWidget(QSharedPointer<SignalVisualisationModel> signal_visualisation_model,
    ...
    if (event_manager_->getFileType().startsWith("XDF", Qt::CaseInsensitive)) {
        ...
        ui_.groupBox->setToolTip(
            "Select or customize an event type then click anywhere on the signals to "
            "create new events");
    } else  // custom event seems doesn't work in files other than XDF
    {
        ...
        ui_.pushButton->hide();
        ui_.groupBox->setToolTip(
            "Select an event type then click anywhere on the signals to create new "
            "events");
    }
}
...
int EventCreationWidget::insertNewEventType() {
    if (ui_.lineEdit->text().compare("")) {
        ...
        for (int i = 0; i < ui_.type_combobox_->count(); i++) {
            if (ui_.lineEdit->text().compare(ui_.type_combobox_->itemText(i), Qt::CaseSensitive)
        ...
        ui_.type_combobox_
            ->insertItem(customized_event_id_, customized_text_, QVariant(customized_event_id_));
