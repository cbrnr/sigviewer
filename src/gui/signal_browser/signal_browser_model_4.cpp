...
SignalBrowserModel::SignalBrowserModel(QSharedPointer<EventManager> event_manager,
    ...
    : SignalVisualisationModel(std::set<EventType>(), channel_manager),
    ...
    if (!event_manager_.isNull()) {
        ...
        if (id2event_item_.size() == 0) {
            for (QList<EventID>::const_iterator event_id_iter = event_ids.begin();
                ...
                ++event_id_iter) {
                id2event_item_[*event_id_iter] = new EventGraphicsItem(*this,
...
void SignalBrowserModel::setShownChannels(std::set<ChannelID> const& new_shown_channels) {
    ...
    foreach (ChannelID channel, channel2signal_item_.keys())
        if (new_shown_channels.count(channel) == 0) removeChannel(channel);
...
void SignalBrowserModel::addChannel(ChannelID channel_id) {
    ...
    signal_item->connect(getSignalViewSettings().data(),
...
void SignalBrowserModel::removeChannel(ChannelID channel_id) {
    ...
    if (sig_iter == channel2signal_item_.end()) return;
...
void SignalBrowserModel::zoomInAll() {
    ...
    for (iter = channel2signal_item_.begin(); iter != channel2signal_item_.end(); iter++) {
        iter.value()->zoomIn();
    }
...
void SignalBrowserModel::update() {
    ...
    double pixel_per_sec =
        getSignalViewSettings()->getPixelsPerSample() * channel_manager_.getSampleRate();
    ...
    for (signal_iter = channel2signal_item_.begin(); signal_iter != channel2signal_item_.end();
        signal_iter++,
        y_pos += getSignalViewSettings()->getChannelHeight()
...
void SignalBrowserModel::selectEvent(EventID id) {
    ...
    if (event_manager_->getEvent(id)->getChannel() == UNDEFINED_CHANNEL)
...
void SignalBrowserModel::updateEventItemsImpl() {
    ...
    for (event_iter = id2event_item_.begin(); event_iter != id2event_item_.end(); event_iter++) {
        QSharedPointer<SignalEvent const> event = event_manager_->getEvent(event_iter->first);
        if (!event) {
            continue;
        }
        ...
        if (!shown_event_types.count(event->getType()) || y_pos_iter == channel2y_pos_.end()) {
...
void SignalBrowserModel::removeEventItem(EventID id) {
    ...
    if (selected_event_item_ == event_item) unselectEvent();
...
void SignalBrowserModel::updateEvent(EventID id) {
    ...
    if (selected_event_item_)
        if (selected_event_item_->getId() == id) {
            if (selected_event_item_->getSignalEvent()->getChannel() == UNDEFINED_CHANNEL)
                tab_context_->setSelectionState(TAB_STATE_EVENT_SELECTED_ALL_CHANNELS);
            else
                tab_context_->setSelectionState(TAB_STATE_EVENT_SELECTED_ONE_CHANNEL);
