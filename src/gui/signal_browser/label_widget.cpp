...
void LabelWidget::paintEvent(QPaintEvent*) {
    ...
    if (channel_overlapping) return;
    if (enable_separator_) {
        for (float32 float_y = 0; float_y <= signal_height * channel_nr2label_.size();
...
void LabelWidget::contextMenuEvent(QContextMenuEvent* event) {
    ...
    menu.addSeparator();
    QAction* visibility_action = menu.addAction(tr("Labels"));
...
void LabelWidget::addChannel(ChannelID channel_nr, const QString& label) {
    ...
    QRect bounding =
        painter.boundingRect(0, 0, 500, 500, Qt::AlignHCenter | Qt::AlignVCenter, label);
...
void LabelWidget::removeChannel(ChannelID channel_nr) {
    ...
    for (QMap<ChannelID, QString>::iterator iter = channel_nr2label_.begin();
        ...
        iter++) {
        QRect bounding =
            p.boundingRect(0, 0, 200, p.font().pixelSize(), Qt::AlignHCenter | Qt::AlignVCenter, iter.value());
