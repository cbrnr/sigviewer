...
void EventGraphicsItem::updateToSignalEvent() {
    ...
    QRectF old_rect;
    if (scene()) old_rect = this->sceneBoundingRect();
    ...
    if (signal_event_->getChannel() == UNDEFINED_CHANNEL) {
        height_ = (signal_view_settings_->getChannelHeight())
                  * signal_browser_model_.getShownChannels().size();

        // if channels are overlapping, adapt the event rectangle accordingly
        if (signal_view_settings_->getChannelOverlapping()) {
            height_ = (height_
                          + (height_ * (signal_browser_model_.getShownChannels().size() - 1)
    ...
    if (scene()) scene()->update(old_rect);
    update();
}
...
void EventGraphicsItem::mouseMoveEvent(QGraphicsSceneMouseEvent* mouse_event) {
    ...
    switch (state_) {
        ...
        case STATE_MOVE_BEGIN: {
            ...
            emit mouseAtSecond(static_cast<float>(pos().x())
                               / (pixel_per_sample * event_manager_->getSampleRate()));
        } break;
        case STATE_MOVE_END: {
            ...
            if (diff > 0)
                scene()->update(pos().x() + width_ - diff - 10, pos().y(), diff + 20, height_);
            else
                scene()->update(pos().x() + width_ - 10, pos().y(), 20 - diff, height_);
            emit mouseAtSecond(static_cast<float>(pos().x() + width_)
                               / (pixel_per_sample * event_manager_->getSampleRate()));
        } break;
        default:
            break;
...
void EventGraphicsItem::mouseReleaseEvent(QGraphicsSceneMouseEvent*) {
    ...
    switch (state_) {
        case STATE_MOVE_BEGIN: {
            ...
            ResizeEventUndoCommand* command =
                new ResizeEventUndoCommand(event_manager_, signal_event_->getId(), sample_pos, dur);
            command_executer_->executeCommand(command);
        } break;
        case STATE_MOVE_END: {
            int32 dur = (static_cast<float32>(width_) / pixel_per_sample) + 0.5;
            ResizeEventUndoCommand* command = new ResizeEventUndoCommand(event_manager_,
                signal_event_->getId(),
...
EventGraphicsItem::Action EventGraphicsItem::getMousePressAction(QGraphicsSceneMouseEvent* e) {
    ...
    switch (SignalBrowserMouseHandling::getAction(e, mode)) {
        case SignalBrowserMouseHandling::SELECT_RESIZE_EVENT_ACTION: {
            ...
            // move event end
            QRect move_end_rect(old_rect.right() - tmp,
                old_rect.top(),
                tmp + move_mouse_range_,
                old_rect.height());
        ...
        case SignalBrowserMouseHandling::SHIFT_EVENT_TO_CHANNEL_ACTION:
            if (this == old_selected_item && old_selected_item->boundingRect().contains(mouse_pos)
