...
SignalGraphicsItem::SignalGraphicsItem(QSharedPointer<SignalViewSettings const> signal_view_settings,
    ...
    connect(signal_view_settings.data(),
        SIGNAL(gridFragmentationChanged()),
        SLOT(updateYGridIntervall()));
...
void SignalGraphicsItem::scaleImpl(double min, double max) {
    ...
    if (y_zoom_ != new_y_zoom || y_offset_ != new_y_offset) {
        y_zoom_ = new_y_zoom;
...
void SignalGraphicsItem::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*) {
    ...
    unsigned length = ((clip.x() - start_sample * pixel_per_sample) + clip.width() + 20);
    if (last_x + length > width_) length = width_ - last_x;

    length /= pixel_per_sample;
    if (length < channel_manager_.getNumberSamples() - start_sample) length++;

    QSharedPointer<DataBlock const> data_block = channel_manager_
                                                     .getData(id_, start_sample, length);

    last_x = start_sample * pixel_per_sample;

    if (draw_x_grid_) drawXGrid(painter, option);

    if (draw_y_grid_) drawYGrid(painter, option);
    ...
    if (pixel_per_sample >= dpr) {
        ...
        for (int index = 0; index < static_cast<int>(data_block->size()); index++) {
            ...
            if (std::isnan(y)) {
                ...
                double clamped_y;
                if (raw_y < oob_above_y)
                    clamped_y = oob_above_y;
                else if (raw_y > oob_below_y)
                    clamped_y = oob_below_y;
                else
                    clamped_y = raw_y;
        ...
        unsigned ds_factor = channel_manager_.getNearestDownsamplingFactor(id_,
            static_cast<unsigned>(samples_per_pixel));
        ...
        for (int px = first_pixel; px <= last_pixel; px++) {
            ...
            if (has_valid) {
                ...
                if (!std::isnan(prev_mid)) {
                    double prev_sy = std::max(YMIN,
                        std::min(YMAX, y_offset_ - y_zoom_ * (double)prev_mid));
                    double cur_sy =
                        std::max(YMIN, std::min(YMAX, y_offset_ - y_zoom_ * (double)cur_mid));
                    painter->drawLine(QPointF(screen_x - 1.0, prev_sy), QPointF(screen_x, cur_sy));
                }

                // Overdraw the precise min–max bar on top.
                painter->drawLine(QPointF(screen_x, sy_max), QPointF(screen_x, sy_min));
...
void SignalGraphicsItem::updateYGridIntervall() {
    ...
    value_range_fragment_ = MathUtils_::round125(
        value_range / signal_view_settings_->getGridFragmentation(Qt::Vertical));
...
void SignalGraphicsItem::mouseMoveEvent(QGraphicsSceneMouseEvent* event) {
    ...
    if (shifting_) {
        ...
        if (new_event_width < 0) new_event_width = 0;

        new_signal_event_->setDuration(new_event_width);

        int32 update_start = std::min(old_pos, new_signal_event_->getPosition());
        int32 update_end = std::max(old_pos + old_width,
            new_event_width + new_signal_event_->getPosition());
        update(update_start, 0, update_end - update_start, height_);

        emit mouseAtSecond(sample_cleaned_pos / pixel_per_sample * channel_manager_.getSampleRate());
    } else
        event->ignore();
}
...
void SignalGraphicsItem::hoverMoveEvent(QGraphicsSceneHoverEvent* event) {
    ...
    if (event_manager_.isNull()) return;
    ...
    for (const auto event : events) {
        ...
        if (shown_types.count(signal_event->getType())) {
            if (event_string.size()) event_string += "<br /><br />";
            event_string += tr("<b>%1</b><br />Start: %2s; Duration: %3s")
                                .arg(event_manager_->getNameOfEvent(event))
                                .arg(signal_event->getPositionInSec())
                                .arg(signal_event->getDurationInSec());
        }
    }
...
void SignalGraphicsItem::mousePressEvent(QGraphicsSceneMouseEvent* event) {
    ...
    switch (SignalBrowserMouseHandling::getAction(event, mode)) {
        ...
        case SignalBrowserMouseHandling::NEW_EVENT_ACTION: {
            if (signal_browser_model_.getShownEventTypes().size() == 0) break;
            ...
            if (event_manager_->getFileType().startsWith("XDF", Qt::CaseInsensitive)) {
                // If user added events in Sigviewer, we will create a new stream to
                // store these events and later store back to the XDF file
                ...
                new_signal_event_ = QSharedPointer<SignalEvent>(new SignalEvent(sample_cleaned_pos,
                    ...
                    XDFdata->userAddedStream,
                    id_));
                // Add the newly created event to the XDFdata object for later use
                QString eventName = event_manager_->getNameOfEventType(
                    signal_browser_model_.getActualEventCreationType());
                XDFdata->userCreatedEvents.emplace_back(eventName.toStdString(),
                    (sample_cleaned_pos / event_manager_->getSampleRate()) + XDFdata->minTS);
            } else {
                new_signal_event_ = QSharedPointer<SignalEvent>(new SignalEvent(sample_cleaned_pos,
                    signal_browser_model_.getActualEventCreationType(),
                    event_manager_->getSampleRate(),
            ...
            new_event_color_ = color_manager_->getEventColor(signal_browser_model_
                    .getActualEventCreationType());
...
void SignalGraphicsItem::mouseReleaseEvent(QGraphicsSceneMouseEvent* event) {
    ...
    if (new_event_) {
        ...
        NewEventUndoCommand* new_event_command = new NewEventUndoCommand(event_manager_,
            new_signal_event_,
            1.0 / signal_view_settings_->getPixelsPerSample());
...
void SignalGraphicsItem::contextMenuEvent(QGraphicsSceneContextMenuEvent* event) {
    ...
    context_menu
        ->addAction(GuiActionFactory::getInstance()->getQAction(tr("Change Color...")));
    context_menu
        ->addAction(GuiActionFactory::getInstance()->getQAction(tr("Scale...")));
    if (signal_browser_model_.getShownChannels().size() > 1) {
        context_menu->addSeparator();
        context_menu->addAction(GuiActionFactory::getInstance()
                ->getQAction(tr("Hide Channel")));
    }
...
void SignalGraphicsItem::drawYGrid(QPainter* painter, QStyleOptionGraphicsItem const* option) {
    ...
    for (float64 y = y_offset_; y < height_ / 2; y += y_grid_pixel_intervall_) {
        if (y > -static_cast<int>(height_ / 2)) {
            painter->drawLine(clip.x(), y, clip.x() + clip.width(), y);
        }
    }

    for (float64 y = y_offset_; y > -static_cast<int>(height_ / 2); y -= y_grid_pixel_intervall_) {
        if (y < height_ / 2) {
            painter->drawLine(clip.x(), y, clip.x() + clip.width(), y);
        }
    }
...
void SignalGraphicsItem::drawXGrid(QPainter* painter, QStyleOptionGraphicsItem const* option) {
    ...
    if (interval_ < 1) return;

    QRectF clip(option->exposedRect);
    //    painter->setPen (QColor(220, 220, 220, 50)); // Qt::lightGray is still too dark and strong.
    painter->setPen(grid_color_);

    if (clip.width() < 1) return;

    int32 x_start = clip.x();
    int32 x_end = clip.x() + clip.width();

    float64 float_x_start = floor((x_start + interval_ / 2) / interval_) * interval_;

    float64 float_x_end = ceil((x_end - interval_ / 2) / interval_) * interval_ + interval_ / 2;

    for (float32 float_x = float_x_start; float_x < float_x_end; float_x += interval_) {
