...
void YAxisWidget::paintEvent(QPaintEvent*) {
    ...
    for (float32 float_y = signal_height / 2;
        ...
        paintYAxisLabels(&painter,
            iter.value()->getYOffset(),
...
void YAxisWidget::contextMenuEvent(QContextMenuEvent* event) {
    ...
    menu.addAction(GuiActionFactory::getInstance()
            ->getQAction(tr("Channels per Page...")));
    menu.addSeparator();
...
#define UPPER_BORDER 0
    ...
    if (y_grid_pixel_intervall < 1) return;
    ...
    // In the following for loop, we are still going to calculate from  the position of
    // 0 even if 0 doesn't appear in the channel. Because in "view option" mode, the
    // channel can be scrolled up and down arbitrarily.
    for (float64 value_y = offset + channelHeight / 2; value_y < channelHeight;
        ...
        // value_range_fragment from value thereafter, so long as they are within
        // channel height from UPPER_BORDER
    ...
    for (float64 value_y = offset + channelHeight / 2 - y_grid_pixel_intervall; value_y > UPPER_BORDER;
        value_y -= y_grid_pixel_intervall) {
        value += value_range_fragment;
...
void YAxisWidget::paintYUnits(QPainter* painter, QString const& unit_string, float64 channelHeight) {
    if (unit_string.size() > 4) {
        painter->rotate(-90);
        painter->drawText(5, 0, width(), channelHeight, Qt::AlignLeft | Qt::AlignVCenter, unit_string);
        painter->rotate(90);
    } else
        painter->drawText(5, 0, width() - 10, channelHeight, Qt::AlignLeft | Qt::AlignVCenter, unit_string);
}
