...
SignalBrowserView::SignalBrowserView(QSharedPointer<SignalVisualisationModel> signal_browser_model,
    ...
    QWidget* parent)
    : QFrame(parent), model_(signal_browser_model), empty_widget_(new QWidget) {
...
void SignalBrowserView::addEventGraphicsItem(EventGraphicsItem* event_graphics_item) {
    ...
    graphics_view_->update();
    connect(event_graphics_item,
        SIGNAL(mouseAtSecond(float64)),
        x_axis_widget_,
        SLOT(changeHighlightTime(float64)));
...
void SignalBrowserView::removeEventGraphicsItem(EventGraphicsItem* event_graphics_item) {
    ...
    graphics_view_->update();
    qDebug() << "SignalBrowserView::removeEventGraphicsItem "
...
void SignalBrowserView::graphicsViewResized(QResizeEvent* event) {
    ...
    if (event->size().height() < 1 || event->oldSize().height() < 1) return;

    channel_height *= static_cast<double>(event->size().height()) / event->oldSize().height();
    if (channel_height < 10) return;
...
void SignalBrowserView::changeEvent(QEvent* event) {
    if (event->type() == QEvent::PaletteChange)
        emit model_->getSignalViewSettings()
            ->labelColorChanged(palette().color(QPalette::WindowText));
...
void SignalBrowserView::initWidgets(QSharedPointer<EventManager> event_manager,
    ...
    if (event_manager.isNull()) {
        ...
        event_creation_widget_->connect(model_.data(),
            SIGNAL(shownEventTypesChanged(std::set<EventType> const&)),
            SLOT(updateShownEventTypes(std::set<EventType> const&)));
        event_editing_widget_->connect(model_.data(),
            SIGNAL(shownEventTypesChanged(std::set<EventType> const&)),
            SLOT(updateShownEventTypes(std::set<EventType> const&)));
        event_editing_widget_->connect(model_.data(),
            SIGNAL(eventSelected(QSharedPointer<SignalEvent const>)),
            SLOT(updateSelectedEventInfo(QSharedPointer<SignalEvent const>)));
    }

    adapt_browser_view_widget_ =
        new AdaptBrowserViewWidget(this, model_->getSignalViewSettings(), y_axis_widget_, x_axis_widget_, label_widget_);

    connect(adapt_browser_view_widget_, SIGNAL(xAxisVisibilityChanged(bool)), SLOT(toggleXWidget(bool)));

    y_axis_widget_->connect(adapt_browser_view_widget_,
        SIGNAL(yAxisVisibilityChanged(bool)),
        SLOT(setVisible(bool)));
    label_widget_->connect(adapt_browser_view_widget_,
        SIGNAL(labelsVisibilityChanged(bool)),
        SLOT(setVisible(bool)));

    connect(horizontal_scrollbar_,
        SIGNAL(valueChanged(int)),
        graphics_view_->horizontalScrollBar(),
        SLOT(setValue(int)));
    connect(graphics_view_->horizontalScrollBar(),
        SIGNAL(valueChanged(int)),
    ...
    connect(graphics_view_->horizontalScrollBar(),
        ...
        this,
        SLOT(horizontalScrollBarRangeChaned(int, int)));
    connect(horizontal_scrollbar_, SIGNAL(valueChanged(int)), this, SLOT(horizontalSrollbarMoved(int)));

    connect(vertical_scrollbar_,
        SIGNAL(valueChanged(int)),
        graphics_view_->verticalScrollBar(),
        SLOT(setValue(int)));
    connect(graphics_view_->verticalScrollBar(),
        SIGNAL(valueChanged(int)),
        vertical_scrollbar_,
        SLOT(setValue(int)));
    connect(graphics_view_->verticalScrollBar(),
        SIGNAL(rangeChanged(int, int)),
        this,
        SLOT(verticalScrollBarRangeChaned(int, int)));
    connect(vertical_scrollbar_, SIGNAL(valueChanged(int)), this, SLOT(verticalSrollbarMoved(int)));

    //! To sync event creation widget and editing widget
    connect(event_creation_widget_,
        SIGNAL(newEventType(std::set<EventType>)),
        event_editing_widget_,
        SLOT(updateShownEventTypes(std::set<EventType>)));

    connect(this, SIGNAL(visibleXChanged(int32)), x_axis_widget_, SLOT(changeXStart(int32)));
    connect(model_->getSignalViewSettings().data(),
        SIGNAL(pixelsPerSampleChanged()),
        x_axis_widget_,
        SLOT(update()));
    ...
    connect(model_->getSignalViewSettings().data(),
        SIGNAL(channelHeightChanged(uint)),
        y_axis_widget_,
        SLOT(changeSignalHeight(uint)));
    connect(model_.data(),
        SIGNAL(modeChanged(SignalVisualisationMode)),
