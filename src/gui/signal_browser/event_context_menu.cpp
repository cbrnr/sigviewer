...
void EventContextMenu::addActionsToMenu(QMenu& menu, EventID event) {
    ...
    if (event_manager_->getEvent(event)->getChannel() != UNDEFINED_CHANNEL) {
        actions
            .append(GuiActionFactory::getInstance()->getQAction(tr("To all Channels")));
        actions.append(GuiActionFactory::getInstance()
                ->getQAction(tr("Copy to Channels...")));
    }
    actions.append(GuiActionFactory::getInstance()->getQAction(tr("Delete")));
    actions
        .append(GuiActionFactory::getInstance()->getQAction(tr("Change Channel...")));
    ...
    if (browser_model_.getShownEventTypes().size() > 1)
        actions.append(GuiActionFactory::getInstance()
                ->getQAction(tr("Hide Events of other Type")));
    else
        actions
            .append(GuiActionFactory::getInstance()->getQAction(tr("Show all Events")));
    actions.append(GuiActionFactory::getInstance()
            ->getQAction(tr("Fit View to Selected Event")));

    foreach (QAction* action, actions) menu.addAction(action);
}
