...
EventEditingWidget::EventEditingWidget(QSharedPointer<EventManager> event_manager,
    ...
    ui_.previous_button_->setDefaultAction(GuiActionFactory::getInstance()
            ->getQAction(tr("Goto and Select Previous Event")));
    ui_.next_button_->setDefaultAction(GuiActionFactory::getInstance()
            ->getQAction(tr("Goto and Select Next Event")));
    ui_.fit_button_->setDefaultAction(GuiActionFactory::getInstance()
            ->getQAction(tr("Fit View to Selected Event")));

    unsigned precision =
        MathUtils_::sampleRateToDecimalPrecision(event_manager_->getSampleRate());
    ui_.begin_spinbox_->setDecimals(precision);
    ui_.begin_spinbox_->setMaximum(static_cast<double>(event_manager_->getMaxEventPosition())
    ...
    if (event_manager_->getFileType().startsWith("XDF", Qt::CaseInsensitive)) {
        ...
        ui_.groupBox->setToolTip(
            tr("Click on any event block on the signals first then edit its type"));
        ui_.groupBox_2
            ->setToolTip(tr("Click on any event block on the signals first then edit "
                            "its starting position"));
        ui_.groupBox_3->setToolTip(
            tr("Click on any event block on the signals first then edit its duration"));
    }
}
...
void EventEditingWidget::updateShownEventTypes(std::set<EventType> const& shown_event_types) {
    ...
    for (const auto type : shown_event_types)
        ui_.type_combobox_->addItem(event_manager_->getNameOfEventType(type), QVariant(type));
...
void EventEditingWidget::updateSelectedEventInfo(QSharedPointer<SignalEvent const> selected_signal_event) {
    ...
    if (selected_signal_event_.isNull())
        ...
        ui_.selection_frame_
            ->setToolTip(tr("Editing events are not allowed in XDF files"));
    } else {
        ui_.selection_frame_->setEnabled(true);
        ui_.type_combobox_->setCurrentIndex(ui_.type_combobox_
                ->findData(QVariant(selected_signal_event_->getType())));
        ui_.duration_spinbox_->setValue(selected_signal_event_->getDurationInSec());
        ui_.duration_spinbox_
            ->setMaximum(static_cast<double>(event_manager_->getMaxEventPosition()
                                             - selected_signal_event->getPosition())
                         / event_manager_->getSampleRate());
        ui_.begin_spinbox_->setValue(selected_signal_event_->getPositionInSec());
        bool has_no_previous_event =
            event_manager_->getPreviousEventOfSameType(selected_signal_event->getId()) == UNDEFINED_EVENT_ID;
        bool has_no_next_event =
            event_manager_->getNextEventOfSameType(selected_signal_event->getId()) == UNDEFINED_EVENT_ID;
...
void EventEditingWidget::on_type_combobox__currentIndexChanged(int combo_box_index) {
    ...
    if (event_type != selected_signal_event_->getType()) {
        ChangeTypeUndoCommand* change_type_command =
            new ChangeTypeUndoCommand(event_manager_, selected_signal_event_->getId(), event_type);
...
void EventEditingWidget::on_begin_spinbox__editingFinished() {
    ...
    unsigned new_begin = ui_.begin_spinbox_->value() * event_manager_->getSampleRate();

    ResizeEventUndoCommand* resize_command = new ResizeEventUndoCommand(event_manager_,
...
void EventEditingWidget::on_duration_spinbox__editingFinished() {
    ...
    unsigned new_duration = ui_.duration_spinbox_->value() * event_manager_->getSampleRate();
