...
bool OpenFileGuiCommand::confirmClosingOldFile() {
    ...
    if (!current_file_context.isNull() && current_file_context->getState() == FILE_STATE_CHANGED) {
...
void OpenFileGuiCommand::evaluateEnabledness() {
    ...
    if (file_opened) {
        if (applicationContext()->getCurrentFileContext()->getFileName().endsWith(
                "xdf"))
            enable_import_events =
                false;  // Disabled because currently XDF files doesn't support importing events
    }
...
void OpenFileGuiCommand::open() {
    ...
    QStringList extension_list = FileSignalReaderFactory::getInstance()
                                     ->getAllFileEndingsWithWildcards();
    QString extensions;
    foreach (QString extension, extension_list) extensions.append(extension + " ");
    QSettings settings;
    QString open_path = settings.value("file_open_path").toString();
    if (!open_path.length()) open_path = QDir::homePath();
...
void OpenFileGuiCommand::importEvents() {
    ...
    int numberChannels =
        applicationContext()->getCurrentFileContext()->getChannelManager().getNumberChannels();
    ...
    if (!serror2(evtHDR)) {
        ...
        for (size_t k = 0; k < NumEvents; k++) {
            ...
            if (pos > event_manager->getMaxEventPosition()
                ...
                || chn > numberChannels || !types.count(typ))
                continue;

            QSharedPointer<SignalEvent> event = QSharedPointer<SignalEvent>(
                new SignalEvent(pos, typ, sampleRate, -1, chn - 1, dur));
...
void OpenFileGuiCommand::openFileImpl(QString file_path, bool instantly) {
    ...
    FileSignalReader* file_signal_reader = FileSignalReaderFactory::getInstance()
                                               ->getHandler(file_path);

    if (file_signal_reader == 0) return;
    ...
    ProgressBar::instance().initAndShow(channel_manager->getNumberChannels() * 3,
        tr("Opening ") + file_name,
        applicationContext());

    QSharedPointer<EventManager> event_manager(new EventManager(*file_signal_reader));
    QSharedPointer<FileContext> file_context(
        new FileContext(file_path, event_manager, channel_manager, file_signal_reader->getBasicHeader()));
...

    {
        ...
        SigViewer_::DownSamplingThread ds_thread(cm,
            2,
    ...
    {
        QSettings s;
        s.setValue("file_open_path", file_path.left(file_path.length() - file_name.length()));
    }
...
QString OpenFileGuiCommand::showOpenDialog(QString const& path, QString const& extensions) {
    ...
    for (QStringList::iterator it = ext_list.begin(); it != ext_list.end(); it++) {
        if (it->size()) extension_selection += ";; " + *it + " (" + *it + ")";
    }
    extension_selection += ";; *.* (*.*)";
    return QFileDialog::getOpenFileName(0, tr("Chose signal file to open"), path, extension_selection);
}
