...
void MouseModeGuiCommand::init() {
    ...
    getQAction(VIEW_OPTIONS_TEXT_())
        ->setIcon(QIcon::fromTheme("tv_options_input_settings"));
...
void MouseModeGuiCommand::evaluateEnabledness() {
    ...
    if (!currentVisModel().isNull()) {
        ...
        getQAction(POINTER_MODE_TEXT_())
            ->setDisabled(currentVisModel()->getEventManager().isNull());
    }
}
