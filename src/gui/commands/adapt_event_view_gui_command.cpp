...
void AdaptEventViewGuiCommand::init() {
    ...
    setIcon(GO_TO_NEXT_EVENT_(), QIcon::fromTheme("chevron_forward"));
    setIcon(GO_TO_PREVIOUS_EVENT_(), QIcon::fromTheme("chevron_backward"));
    setIcon(SET_SHOWN_EVENTS_(), QIcon::fromTheme("label"));
    setIcon(FIT_TO_EVENT_(), QIcon::fromTheme("zoom_out_map"));
...
void AdaptEventViewGuiCommand::evaluateEnabledness() {
    ...
    disableIfNoEventSelected(QStringList() << HIDE_EVENTS_OF_OTHER_TYPE_() << FIT_TO_EVENT_());
...
void AdaptEventViewGuiCommand::gotoAndSelectEvent(bool forward) {
    ...
    if (event_manager.isNull()) {
        qCritical() << tr(
            "AdaptEventViewGuiCommand::gotoAndSelectEvent no EventManager available!!");
        return;
    }
    ...
    if (!event.isNull()) {
        GuiHelper::animateProperty(currentVisModel().data(),
            "sample_position_",
...
void AdaptEventViewGuiCommand::setNextAndPreviousEvent() {
    ...
    QSharedPointer<SignalEvent const> event = events.first();
    if (!event.isNull()) {
        if (current_event_ == event->getId()) return;
