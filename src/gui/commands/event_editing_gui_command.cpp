...
void EventEditingGuiCommand::deleteSelectedEvent() {
    ...
    foreach (EventID event, GuiHelper::getSelectedEventIDs(currentEventView())) {
        commands.append(QSharedPointer<QUndoCommand>(
            new DeleteEventUndoCommand(currentEventView()->getEventManager(), event)));
    }
...
void EventEditingGuiCommand::changeTypeSelectedEvent() {
    ...
    foreach (EventID event, GuiHelper::getSelectedEventIDs(currentEventView())) {
        ...
        if (new_type == UNDEFINED_EVENT_TYPE) return;

        commands.append(QSharedPointer<QUndoCommand>(
            new ChangeTypeUndoCommand(event_manager, event, new_type)));
    }
...
void EventEditingGuiCommand::changeChannelSelectedEvent() {
    ...
    foreach (EventID event, GuiHelper::getSelectedEventIDs(currentEventView())) {
        ...
        ChannelID new_channel = GuiHelper::selectChannel(preselected_channel, currentVisModel());

        if (new_channel == preselected_channel) return;

        commands.append(QSharedPointer<QUndoCommand>(
            new ChangeChannelUndoCommand(event_manager, event, new_channel)));
    }
    executeCommands(commands);
}
...
void EventEditingGuiCommand::toAllChannelsSelectedEvent() {
    ...
    foreach (EventID event, GuiHelper::getSelectedEventIDs(currentEventView())) {
        ...
        if (event_manager->getEvent(event)->getChannel() == UNDEFINED_CHANNEL) return;

        commands.append(QSharedPointer<QUndoCommand>(
            new ChangeChannelUndoCommand(event_manager, event, UNDEFINED_CHANNEL)));
    }
    executeCommands(commands);
}
...
void EventEditingGuiCommand::copyToChannelsSelectedEvent() {
    ...
    foreach (EventID event_id, GuiHelper::getSelectedEventIDs(currentEventView())) {
        ...
        std::set<ChannelID> channels =
            GuiHelper::selectShownChannels(event->getChannel(), currentVisModel());
        ...
        for (std::set<ChannelID>::iterator channel_iter = channels.begin();
            ...
            QSharedPointer<QUndoCommand> command(
                new NewEventUndoCommand(event_manager, new_event, 1));
...
void EventEditingGuiCommand::insertEventOverSelectedEvent() {
    QList<QSharedPointer<QUndoCommand> > commands;
    foreach (EventID event, GuiHelper::getSelectedEventIDs(currentEventView())) {
        if (event == UNDEFINED_EVENT_ID) return;
...
void EventEditingGuiCommand::evaluateEnabledness() {
    ...
    bool event_one_channel_selected =
    ...
    disableIfNoEventSelected(QStringList() << INSERT_OVER_() << CHANGE_CHANNEL_()
