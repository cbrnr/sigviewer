...
void SaveGuiCommand::saveAs() {
    ...
    QString new_file_path =
        GuiHelper::getFilePathFromSaveAsDialog(old_file_path, extension, tr("Signal files"));
    ...
    if (new_file_path.size()) {
        ...
        if (new_file_path.endsWith("xdf", Qt::CaseInsensitive)) {
            ...
            FileSignalWriter* writer(FileSignalWriterFactory::getInstance()
                    ->getHandler(new_file_path));

            bool can_save_events = false;
            if (writer) can_save_events = writer->supportsSavingEvents();

            QSharedPointer<EventManager> event_mgr = file_context->getEventManager();
            if (writer && can_save_events) {
                QString error =
                    writer->saveEventsToSignalFile(event_mgr, event_mgr->getEventTypes());
                if (error.size())
                ...
                QMessageBox::information(0,
                    "",
                    tr("Events not stored to %1\n If you want to store events export "
                       "the file to GDF or export the events into a EVT file!")
                        .arg(new_file_path));
            delete writer;
        }
    } else
...
void SaveGuiCommand::save() {
    ...
    } else  // Original Sigviewer code (not XDF)
    {
        ...
        if (writer && can_save_events) {
            ...
            QMessageBox::StandardButton pressed_button = QMessageBox::question(0,
                file_name,
                tr("Saving of Events is not possible to this file format! Do you want "
                   "to convert this file into GDF?"),
                QMessageBox::Yes | QMessageBox::No,
                QMessageBox::Yes);
            if (pressed_button == QMessageBox::Yes) exportToGDF();
        }
        delete writer;
    }
}
...
void SaveGuiCommand::exportToPNG() {
    ...
    QString file_path = GuiHelper::getFilePathFromSaveAsDialog(
        applicationContext()->getCurrentFileContext()->getFilePath(),
        extension,
        tr("PNG files"));
...
void SaveGuiCommand::exportToGDF() {
    ...
    current_file_path = current_file_path
                            .left(current_file_path.size() - current_file_name.size());

    QString new_file_path =
        GuiHelper::getFilePathFromSaveAsDialog(current_file_path, extensions, tr("GDF files"));

    if (new_file_path.size() == 0) return;
    ...
    QString error = writer->save(applicationContext()->getCurrentFileContext(),
        event_mgr->getEventTypes());
    if (error.size() == 0) {
        QMessageBox::StandardButton pressed_button = QMessageBox::question(0,
            current_file_name,
            tr("%1 has been converted into GDF and stored in:\n%2\n\nDo you want to "
               "open the GDF file?")
...
void SaveGuiCommand::exportEventsToEVT() {
    ...
    QString new_file_path = GuiHelper::getFilePathFromSaveAsDialog(
        current_file_path.left(current_file_path.lastIndexOf('.')) + extension,
        extensions,
        tr("Events files"));
...
void SaveGuiCommand::exportEventsToCSV() {
    ...
    if (file.is_open()) {
        ...
        std::sort(events.begin(), events.end(), [](const row& a, const row& b) {
            return a.pos < b.pos;
        });

        for (int i = 0; i < events.size(); ++i) {
            file << events[i].pos << "," << events[i].dur << "," << events[i].chan << ","
                 << events[i].id << "," << events[i].name.remove(",").toStdString() << "\n";
        }
        file.close();
    } else {
...
void SaveGuiCommand::evaluateEnabledness() {
    ...
    if (file_open) {
        no_gdf_file_open =
            !(
                applicationContext()->getCurrentFileContext()->getFileName().endsWith("gdf"));
        file_changed = (getFileState() == FILE_STATE_CHANGED);
        has_events = applicationContext()->getCurrentFileContext()->getEventManager()->getNumberOfEvents()
                     > 0;

        if (applicationContext()->getCurrentFileContext()->getFileName().endsWith(
                "xdf"))
