...
class GuiActionCommandException : public Exception {
   public:
    explicit GuiActionCommandException(QString const& action_id, QString const& error)
        : Exception(QObject::tr("GuiActionCommand: \"%1\" failed: %2")
                  .arg(action_id)
                  .arg(error)
                  .toStdString()) {}
};
...
void GuiActionCommand::initConnections() {
    ...
    qDebug() << "GuiActionCommand::GuiActionCommand connecting to "
                "ApplicationContext::getInstance() = "
             << ApplicationContext::getInstance();
    if (!connect(ApplicationContext::getInstance().data(),
            SIGNAL(stateChanged(ApplicationState)),
            SLOT(updateEnablednessToApplicationState(ApplicationState))))
        throw(GuiActionCommandException(firstActionId,
            tr("connect to signal stateChanged(ApplicationState)")));
    else
        qDebug() << "GuiActionCommand::GuiActionCommand connect to signal "
                    "stateChanged(ApplicationState) = true";
    if (!connect(ApplicationContext::getInstance().data(),
    ...
    if (!connect(ApplicationContext::getInstance().data(),
        ...
        throw(GuiActionCommandException(firstActionId,
            tr("connect to signal currentTabEditStateChanged(TabEditState)")));
    if (!connect(ApplicationContext::getInstance().data(),
...
QSharedPointer<EventView> GuiActionCommand::currentEventView() {
    ...
    QSharedPointer<MainWindowModel> main_window_model = ApplicationContext::getInstance()
                                                            ->getMainWindowModel();
...
QSharedPointer<SignalViewSettings> GuiActionCommand::currentSignalViewSettings() {
    ...
    if (!vis_model.isNull()) settings = vis_model->getSignalViewSettings();
...
bool GuiActionCommand::disableIfNoEventsPossible(QStringList const& actions) {
    ...
    if (!currentEventView().isNull()) {
        if (currentEventView()->getEventManager().isNull()) no_events_possible = true;
    } else
