...
class EventTableItem : public QTableWidgetItem {
   public:
    EventTableItem(float32 number, int precision = 0)
...
EventTableWidget::EventTableWidget(QSharedPointer<TabContext> tab_context,
    ...
    ui_.setupUi(this);
    connect(event_manager_.data(),
        SIGNAL(eventCreated(QSharedPointer<const SignalEvent>)),
        SLOT(addToTable(QSharedPointer<const SignalEvent>)));
    ...
    toolbar->addAction(GuiActionFactory::getInstance()
            ->getQAction("Export Events to CSV..."));
    toolbar->addAction(GuiActionFactory::getInstance()
            ->getQAction("Export Events to EVT..."));
...
QSharedPointer<EventView> EventTableWidget::getEventView() {
    if (event_table_view_model_.isNull())
        event_table_view_model_ =
            QSharedPointer<EventTableViewModel>(new EventTableViewModel(*this));
...
void EventTableWidget::addToTable(QSharedPointer<SignalEvent const> event) {
    ...
    QTableWidgetItem* channel_item = new QTableWidgetItem(channel_manager_
            .getChannelLabel(event->getChannel(), event->getStream()));
    QTableWidgetItem* type_item =
        new QTableWidgetItem(event_manager_->getNameOfEvent(event->getId()));
    ...
    ui_.event_table_->setItem(row, POSITION_INDEX_, position_item);
    row = ui_.event_table_->row(position_item);
    ui_.event_table_->setItem(row, DURATION_INDEX_, duration_item);
    row = ui_.event_table_->row(duration_item);
    ui_.event_table_->setItem(row, CHANNEL_INDEX_, channel_item);
    row = ui_.event_table_->row(channel_item);
    ui_.event_table_->setItem(row, TYPE_INDEX_, type_item);
...
void EventTableWidget::updateEventEntry(EventID event_id) {
    ...
    bool updated = false;
    for (int row = 0; (row < ui_.event_table_->rowCount()) && (!updated); row++) {
        QTableWidgetItem* id_item = ui_.event_table_->item(row, ID_INDEX_);
        if (id_item->text().toInt() == event_id) {
            ui_.event_table_->item(row, CHANNEL_INDEX_)
                ->setText(channel_manager_
                        .getChannelLabel(event->getChannel(), event->getStream()));
            ui_.event_table_->item(row, POSITION_INDEX_)
                ->setText(QString::number(event->getPositionInSec(), 'f', precision_));
            ui_.event_table_->item(row, DURATION_INDEX_)
                ->setText(QString::number(event->getDurationInSec(), 'f', precision_));
            ui_.event_table_->item(row, TYPE_INDEX_)
                ->setText(event_manager_->getNameOfEvent(event_id));
            updated = true;
        }
    }
}
