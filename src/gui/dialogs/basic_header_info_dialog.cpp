...
// constructor
BasicHeaderInfoDialog::BasicHeaderInfoDialog(QSharedPointer<BasicHeader> header, QWidget* parent)
    : QDialog(parent), basic_header_(header) {
...
void BasicHeaderInfoDialog::buildTree() {
    ...
    //    tmp_item->setText(1, basic_header_->getRecordingTime().toString("dd.MM.yyyy
    //    hh:mm:ss")); tmp_item = new QTreeWidgetItem(root_item);
    ...
    if (basic_header_->getFileTypeString().startsWith("XDF", Qt::CaseInsensitive)) {
        for (size_t i = 0; i < XDFdata->streams.size(); i++) {
            ...
            for (QDomNode n = rootElement.firstChild(); !n.isNull();) {
                ...
                if (!n.firstChild().isElement()) {
                    ...
                    while (n.nextSibling().isNull() && n.parentNode() != rootElement) {
                        n = n.parentNode();
        ...
        for (uint32 channel_nr = 0; channel_nr < basic_header_->getNumberChannels(); channel_nr++) {
            ...
            channel_item->setText(0,
                tr("Channel %1 (%2)")
                    .arg(channel_nr + 1)  // +1 for 1-based indexing
