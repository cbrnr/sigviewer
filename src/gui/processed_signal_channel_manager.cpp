...
ProcessedSignalChannelManager::ProcessedSignalChannelManager(float64 sample_rate,
    unsigned length,
    QObject* parent)
    : QObject(parent), sample_rate_(sample_rate), length_(length) {
    // nothing to do here
}

void ProcessedSignalChannelManager::addChannel(ChannelID id,
...
ChannelID ProcessedSignalChannelManager::addExtraChannel(ChannelID id,
    QSharedPointer<DataBlock const> data_block,
    QString const& label,
    QString const& y_unit_string) {
    id += 0.1f;
...
std::set<ChannelID> ProcessedSignalChannelManager::getChannels() const {
    ...
    foreach (ChannelID id, channels_.keys()) channels.insert(id);
    return channels;
}
...
size_t ProcessedSignalChannelManager::getNumberSamples() const { return length_; }

float64 ProcessedSignalChannelManager::getSampleRate() const { return sample_rate_; }
