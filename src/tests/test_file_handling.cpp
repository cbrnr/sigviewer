...
class TestFileHandling : public QObject {
   ...
   private slots:
    ...
    void init() { OpenFileGuiCommand::openFile("blub.sinusdummy"); }
    ...
    void exportImportEvents() {
        ...
        QVERIFY(f1.open());
        f1.close();
        QSharedPointer<FileSignalWriter> writer1(FileSignalWriterFactory::getInstance()
                ->getHandler(f1.fileName()));
        QVERIFY(!writer1.isNull());
        QVERIFY(writer1->save(ctx).isEmpty());

        QSharedPointer<FileSignalReader> reader1(FileSignalReaderFactory::getInstance()
                ->getHandler(f1.fileName()));
        QVERIFY(!reader1.isNull());
        QCOMPARE(reader1->getEvents().size(), static_cast<int>(evtMgr->getNumberOfEvents()));

        // Export a single event type
        QTemporaryFile f2("XXXXXX.evt");
        QVERIFY(f2.open());
        f2.close();
        QSharedPointer<FileSignalWriter> writer2(FileSignalWriterFactory::getInstance()
                ->getHandler(f2.fileName()));
        ...
        QSharedPointer<FileSignalReader> reader2(FileSignalReaderFactory::getInstance()
                ->getHandler(f2.fileName()));
        auto events2 = reader2->getEvents();
        QCOMPARE(events2.size(), evtMgr->getEvents(*types.begin()).size());

        for (EventID eid : evtMgr->getEvents(*types.begin())) {
            bool found = false;
            for (auto& ev : events2)
                if (ev->equals(*evtMgr->getEvent(eid))) found = true;
            QVERIFY(found);
        }
    }
};
