...
QString FileHandlingTests::testEventExportingImporting ()
{
    ...
    event_file_2.open();
    event_file_2.close();
    writer = FileSignalWriterFactory::getInstance()->getHandler(event_file_2.fileName());
    VERIFY (writer.isNull() == false, "EVT writer 2 not created")
    std::set<EventType> exported_event_types;
    exported_event_types.insert (event_manager->getEvent(0)->getType());
    error = writer->save (applicationContext()->getCurrentFileContext(), exported_event_types);
    VERIFY (error.size() == 0, error)
    reader = FileSignalReaderFactory::getInstance()->getHandler(event_file_2.fileName());
