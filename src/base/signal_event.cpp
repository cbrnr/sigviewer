...
// constructor
SignalEvent::SignalEvent(size_t position,
    EventType type,
    float64 sample_rate,
    int streamNumber,
    ChannelID channel,
...
// get id
int32 SignalEvent::getId() const { return id_; }
...
// get type
uint16 SignalEvent::getType() const { return type_; }

// get channel
ChannelID SignalEvent::getChannel() const { return channel_; }
...
// set position
void SignalEvent::setPosition(size_t position) { position_ = position; }

// set type
void SignalEvent::setType(EventType type) { type_ = type; }

// set channel
void SignalEvent::setChannel(ChannelID channel) { channel_ = channel; }
...
bool SignalEvent::equals(SignalEvent const& event) const {
    ...
    if (position_ != event.position_) return false;
    if (sample_rate_ != event.sample_rate_) return false;
    if (channel_ != event.channel_) return false;
    if (stream_ != event.stream_) return false;
