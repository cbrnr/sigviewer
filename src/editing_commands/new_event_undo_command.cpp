...
NewEventUndoCommand::NewEventUndoCommand(QSharedPointer<EventManager> event_manager,
    ...
    : event_manager_(event_manager), raw_signal_event_(new SignalEvent(*signal_event)) {
...
void NewEventUndoCommand::redo() {
    ...
    created_signal_event_ = event_manager_->createEvent(raw_signal_event_->getChannel(),
