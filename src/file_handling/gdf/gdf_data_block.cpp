...
std::pair<QSharedPointer<DataBlock>, unsigned> GDFDataBlock::getNearbyDownsampledBlock(
    ...
    for (nearest_factor = downsampling_factor + 1; search && (nearest_factor > 1); --nearest_factor)
        if (downsampled_map_.contains(nearest_factor - 1)) search = false;

    if ((nearest_factor <= downsampling_factor) && (nearest_factor > 1)) {
        return std::pair<QSharedPointer<DataBlock>, unsigned>(
            downsampled_map_[nearest_factor]
                ->createSubBlock(start_sample_ / nearest_factor, size() / nearest_factor),
            nearest_factor);
    } else
