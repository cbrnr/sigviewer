...
QSharedPointer<DataBlock const> GDFFileSignalReader::getSignalData(ChannelID channel_id,
    ...
    if (channel_map_.size() == 0) {
        ...
        for (unsigned channel_index = 0;
            channel_index < reader_->getMainHeader_readonly().get_num_signals();
            channel_index++) {
            unsigned current_channel_length =
                reader_->getSignalHeader_readonly(channel_index).get_samples_per_record()
                * reader_->getMainHeader_readonly().get_num_datarecords();
            max_channel_length = std::max(max_channel_length, current_channel_length);
            QSharedPointer<GDFDataBlock> data(new GDFDataBlock(cache_,
                channel_index,
        ...
        downsampling_thread_ =
            new DownSamplingThread(data_for_downsampling, 3, max_channel_length / 2000);
...
QList<QSharedPointer<SignalEvent const> > GDFFileSignalReader::getEvents() const {
    try {
        ...
        if (events_.size()) return events_;

        std::vector<gdf::Mode3Event> mode_3_events;
        if (reader_->getEventHeader()->getMode() == 1)
            mode_3_events = gdf::convertMode1EventsIntoMode3Events(
                reader_->getEventHeader()->getMode1Events());
        else
        ...
        for (unsigned index = 0; index < mode_3_events.size(); index++) {
            ...
            QSharedPointer<SignalEvent const> event(
                new SignalEvent(gdf_event.position / header_->getDownSamplingFactor(),
                    gdf_event.type,
                    reader_->getEventHeader()->getSamplingRate() / header_->getDownSamplingFactor(),
                    gdf_event.channel - 1,
                    gdf_event.duration / header_->getDownSamplingFactor()));
...
QString GDFFileSignalReader::open(QString const& file_path) {
    ...
    header_ = QSharedPointer<GDFBasicHeader>(
        new GDFBasicHeader(file_path, reader_->getHeaderAccess_readonly()));
