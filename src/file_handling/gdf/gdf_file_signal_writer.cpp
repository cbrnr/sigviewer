...
QString GDFFileSignalWriter::save(QSharedPointer<FileContext const> file_context,
    ...
    gdf::Writer writer;
    writer.getMainHeader().set_recording_id(
        file_context->getFileName().append(" converted to GDF2").toStdString());
    ...
    for (const auto channel : channel_manager.getChannels()) {
        ...
        writer.getSignalHeader(channel)
            .set_label(channel_manager.getChannelLabel(channel).toStdString());
    ...
    try {
        ...
        if (exception.warnings.size() > 0) {
            foreach (std::string error, exception.errors) errors.append(error.c_str());
    ...
    try {
        ...
        for (unsigned sample_index = 0; sample_index < channel_manager.getNumberSamples();
            sample_index++) {
            for (const auto channel_id : channel_manager.getChannels()) {
                writer.addSamplePhys(channel_id,
                    (*(channel_manager.getData(channel_id, sample_index, 1)))[0]);
            }
        ...
        if (!event_manager.isNull()) {
            foreach (EventID event_id, event_manager->getAllEvents()) {
                ...
                writer.addEvent(event->getPosition() + 1,
                    event->getType(),
                    event->getChannel() + 1,
                    event->getDuration());
                ProgressBar::instance().increaseValue(1, progressbar_string);
