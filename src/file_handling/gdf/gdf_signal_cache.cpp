...
float32 GDFSignalCache::getSample(ChannelID channel, unsigned sample_index) {
    ...
    return channel_buffers_[cache_index][channel]
...
void GDFSignalCache::rebuildCache(unsigned cache_index, ChannelID /*channel*/, unsigned sample_index) {
    ...
    qDebug() << "GDFSignalCache::rebuildCache " << cache_index << " for thread "
             << thread_cache_index_map_.key(cache_index) << "; start = " << start;
    ...
    for (int index = 0; index < channel_buffers_[cache_index].size(); index++) {
        ...
        size_t end = std::min<size_t>(start + CACHE_SIZE_PER_CHANNEL_,
            reader_->getSignalHeader_readonly(index).get_samples_per_record()
...
unsigned GDFSignalCache::createNewThreadCacheIndex() {
    ...
    qDebug() << "GDFSignalCache::createNewThreadCacheIndex for thread "
             << QThread::currentThread() << "; new cache index = " << new_cache_index;
