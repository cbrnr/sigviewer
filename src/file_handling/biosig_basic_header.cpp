...
BiosigBasicHeader::BiosigBasicHeader(HDRTYPE* raw_header, QString const& file_path)
    ...
    setFileTypeString(QString(GetFileTypeString(raw_header->TYPE))
            .append(" v")
            .append(QString::number(raw_header->VERSION)));
...
BiosigBasicHeader::BiosigBasicHeader(QString file_format, QString const& file_path)
    : BasicHeader(file_path), number_samples_(XDFdata->totalLen) {
    if (XDFdata->dictionary.size()) {
        for (unsigned index = 0; index < XDFdata->dictionary.size(); index++) {
            // below we use index+1 because in SigViewer, 0 is reserved for a special
            // event type. thus we count from 1
            user_defined_event_map_[index + 1] =
                QString::fromStdString(XDFdata->dictionary[index]);
        }
    }
...
void BiosigBasicHeader::readPatientInfo(HDRTYPE const* raw_header) {
    switch (raw_header->Patient.Handedness) {
        case 1:
            addPatientInfo(QObject::tr("Handedness"), QObject::tr("Right"));
            break;
        case 2:
            addPatientInfo(QObject::tr("Handedness"), QObject::tr("Left"));
            break;
        case 3:
            addPatientInfo(QObject::tr("Handedness"), QObject::tr("Equal"));
            break;
    }

    switch (raw_header->Patient.Sex) {
        case 1:
            addPatientInfo(QObject::tr("Sex"), QObject::tr("Male"));
            break;
        case 2:
            addPatientInfo(QObject::tr("Sex"), QObject::tr("Female"));
            break;
    }

    switch (raw_header->Patient.Smoking) {
        case 1:
            addPatientInfo(QObject::tr("Smoking"), QObject::tr("No"));
            break;
        case 2:
            addPatientInfo(QObject::tr("Smoking"), QObject::tr("Yes"));
            break;
    }
    switch (raw_header->Patient.Smoking) {
        case 1:
    ...
    if (raw_header->Patient.Weight)
        addPatientInfo(QObject::tr("Weight"),
            QString::number(raw_header->Patient.Weight).append("kg"));
    if (raw_header->Patient.Height)
        addPatientInfo(QObject::tr("Height"),
