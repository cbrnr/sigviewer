...
void ChannelManager::addDownsampledMinMaxVersion(ChannelID id,
...
unsigned ChannelManager::getNearestDownsamplingFactor(ChannelID id, unsigned factor) const {
    ...
    for (nearest_factor = factor + 1; search && (nearest_factor > 1); --nearest_factor)
        if (downsampled_min_map_[id].contains(nearest_factor - 1)) search = false;
...
float64 ChannelManager::getMinValue(std::set<ChannelID> const& channels) const {
    ...
    for (const auto channel : channels) min = std::min(min, min_values_[channel]);
    return min;
}
...
float64 ChannelManager::getMaxValue(std::set<ChannelID> const& channels) const {
    ...
    for (const auto channel : channels) max = std::max(max, max_values_[channel]);
    return max;
}
...
void ChannelManager::markMinMaxInitialized() { min_max_initialized_ = true; }

void ChannelManager::initMinMax() const {
    if (min_max_initialized_) return;
