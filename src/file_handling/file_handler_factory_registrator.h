...
#define FILE_SIGNAL_WRITER_REGISTRATION(file_ending, writer)                                      \
    ...
    FileSignalWriterFactoryRegistrator file_writer_registrator##writer##file_ending(#file_ending, \
        QSharedPointer<FileSignalWriter>(new writer));                                            \
    }

#define FILE_SIGNAL_READER_REGISTRATION(file_ending, reader)                                      \
    namespace Registrators_ {                                                                     \
    FileSignalReaderFactoryRegistrator file_reader_registrator##reader##file_ending(#file_ending, \
        QSharedPointer<FileSignalReader>(new reader));                                            \
    }

#define FILE_SIGNAL_READER_DEFAULT_REGISTRATION(reader)                 \
    namespace Registrators_ {                                           \
    FileSignalReaderFactoryRegistrator file_reader_registrator##reader( \
        QSharedPointer<FileSignalReader>(new reader));                  \
    }
