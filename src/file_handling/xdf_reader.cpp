...
QSharedPointer<DataBlock const> XDFReader::getSignalData(ChannelID channel_id,
    ...
    if (!buffered_all_channels_) bufferAllChannels();

    if (!channel_map_.contains(channel_id)) return QSharedPointer<DataBlock const>(0);

    if (length == basic_header_->getNumberOfSamples() && start_sample == 0)
...
QList<QSharedPointer<SignalEvent const> > XDFReader::getEvents() const {
    ...
    if (!buffered_all_events_) bufferAllEvents();
...
QString XDFReader::loadFixedHeader(const QString& file_path) {
    ...
    if (QFile::exists(file_path))  // Double check whether file exists
    {
        if (XDFdata->load_xdf(file_path.toStdString()) == 0) {
                    ...
                    if (XDFdata->effectiveSampleRateVector.size()) {
                        ...
                        XDFdata->fileEffectiveSampleRate =
                            std::accumulate(XDFdata->effectiveSampleRateVector.begin(),
                                XDFdata->effectiveSampleRateVector.end(),
                                init)
            ...
            t = clock() - t - t2;
            qDebug() << "it took " << ((float)t) / CLOCKS_PER_SEC
                     << " additional seconds loading XDF header";
            ...
            if (showWarning)
                QMessageBox::warning(0, "SigViewer", QObject::tr("The effective sampling rate of at least one stream is significantly different than the reported nominal sampling rate. Signal visualization might be inaccurate."), QMessageBox::Ok, QMessageBox::Ok);

            basic_header_ = QSharedPointer<BasicHeader>(new BiosigBasicHeader("XDF", file_path));
...
int XDFReader::setStreamColors() {
    ...
    QVector<QColor> colorList = {"#0055ff",
        "#00aa00",
        "#aa00ff",
        "#00557f",
        "#5555ff",
    ...
    for (auto& stream : XDFdata->streams) {
        if (stream.info.nominal_srate != 0 && stream.info.channel_format.compare("string"))  // filter the string streams
        {
            int startingPosition =
                (stream.info.first_timestamp - XDFdata->minTS) * XDFdata->majSR;
        ...
        // else if: irregualar samples
            ...
            for (auto& row : stream.time_series) {
                ...
                for (size_t i = 0; i < row.size(); i++) {
                    ...
                    float* pt =
                        raw_data->data()
                        + (int)(round((stream.time_stamps[i] - XDFdata->minTS) * XDFdata->majSR));
                    *pt = row[i];
                    ...
                    if (i != stream.time_stamps.size() - 1) {
                        ...
                        int interval = round((stream.time_stamps[i + 1] - stream.time_stamps[i])
                                             * XDFdata->majSR);
                        for (int interpolation = 1; interpolation <= interval; interpolation++) {
                            *(pt + interpolation) =
                                row[i] + interpolation * ((row[i + 1] - row[i])) / (interval + 1);
                        }
                    }
                }
                QSharedPointer<DataBlock const> data_block(
...
void XDFReader::bufferAllEvents() const {
    ...
    for (unsigned index = 0; index < number_events; index++) {
        QSharedPointer<SignalEvent> event(new SignalEvent(
            round((XDFdata->eventMap[index].first.second - XDFdata->minTS) * eventSampleRate),
