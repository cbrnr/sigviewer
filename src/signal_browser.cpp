...

{
    ...
    top_layout->addWidget(y_axis_widget_, 1, 1);

    top_layout->addWidget(canvas_view_, 1, 2);

    top_layout->addWidget(label_widget_, 1, 3);

    top_layout->addWidget(x_axis_widget_, 2, 2);

    top_layout->addWidget(h_scrollbar_, 3, 1, 1, 3);

    top_layout->addWidget(v_scrollbar_, 1, 4, 2, 1);

    QWidget* dummy = new QWidget(this);

    dummy->setBackgroundColor(XAxisWidget::getAxisColor());

    top_layout->addWidget(dummy, 2, 1);

    dummy = new QWidget(this);

    dummy->setBackgroundColor(XAxisWidget::getAxisColor());

    top_layout->addWidget(dummy, 2, 3);
