...
NotchFilter8::NotchFilter8(double fs, double freq)
    ...
    if (!freq) throw(std::invalid_argument("Filter freq must not be 0!"));

    if (!fs) throw(std::invalid_argument("Fs must not be 0!"));
    ...
    if (v_comp.imag())
        throw(std::runtime_error(
            "Coefficient factor has imaginary part -- something went wrong!"));
