...
inline complex<T1> operator*(complex<T1> r, complex<T2> l) {
    complex<T1> x;
...
//!
template <class T>
inline complex<T> operator+(complex<T> r, T l) {
    complex<T> x;
    x.re = r.re + l;
    x.im = r.im;
    return x;
...
//!
template <class T>
inline complex<T> operator+(T r, complex<T> l) {
    complex<T> x;
    x.re = r + l.re;
    x.im = l.im;
    return x;
...
//!
template <class T>
inline complex<T> operator-(complex<T> r) {
    complex<T> x;
    x.re = -r.re;
    x.im = -r.im;
    return x;
...
//!
template <class T>
complex<T> operator/(complex<T> l, T r) {
    complex<T> x(0, 0);
    if (r != 0) {
        x.re = l.re / r;
        x.im = l.im / r;
...
//!
template <class T1, class T2>
complex<T1> operator/(complex<T1> r, complex<T2> l) {
    complex<T1> x;
    T2 den;
    den = magsq(l);
    x = (r * conj(l)) / den;
...
//! Conjugate
template <class T>
inline complex<T> conj(complex<T> x) {
    complex<T> y;
    y.re = x.re;
    y.im = -x.im;
    return y;
...
template <class T>
inline complex<double> norm(complex<T> x) {
    T y;
...
template <class T>
inline complex<T> maxvalue(complex<T> x1) {
    return (complex<T>(MAX(x1.re, x1.im)));
};

template <class T>
inline complex<T> minimum(complex<T> x1, complex<T> x2) {
    return (complex<T>(MIN(x1.re, x2.re), MIN(x1.im, x2.im)));
}
template <class T>
inline complex<T> maximum(complex<T> x1, complex<T> x2) {
...
inline complex<long> saturate(complex<long> in, long bits) {
    ...
    if (labs(in.re) > low_mask)
        out.re = (in.re > 0) ? low_mask : ~low_mask;
    else
        out.re = in.re;
    if (labs(in.im) > low_mask)
        out.im = (in.im > 0) ? low_mask : ~low_mask;
    else
        out.im = in.im;
    return (out);
}
template <class T>
complex<T> signbit(complex<T> in) {
...
//! squaring function
template <class T>
inline complex<T> sqr(complex<T> x) {
    return (x * x);
};
//! square root function (same as sqrt in spuc_math.h)
template <class T>
inline complex<double> csqrt(complex<T> x) {
