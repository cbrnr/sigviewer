	...
	if (file.is_open())
	{
		...
		while (1)
		{
			...
			case 3: //read [Samples] chunk
			{
                ...
                //allocate space
                XDFdata.streams[index].time_stamps.reserve(numSamp);

                /*if (XDFdata.streams[index].info.channel_format.compare("string") == 0)
                {
                    XDFdata.streams[index].time_seriesStr.resize(XDFdata.streams[index].info.channel_count);
                    for (auto &elem : XDFdata.streams[index].time_seriesStr)
                    {
                        elem.reserve(numSamp);
                    }
                }*/

                if (XDFdata.streams[index].info.channel_format.compare("double64") == 0 ||
                         XDFdata.streams[index].info.channel_format.compare("float32") == 0 ||
                         XDFdata.streams[index].info.channel_format.compare("int8") == 0 ||
                         XDFdata.streams[index].info.channel_format.compare("int16") == 0 ||
                         XDFdata.streams[index].info.channel_format.compare("int32") == 0 ||
                         XDFdata.streams[index].info.channel_format.compare("int64") == 0)
                {
                    XDFdata.streams[index].time_series.resize(XDFdata.streams[index].info.channel_count);
                    for (auto &elem : XDFdata.streams[index].time_series)
                    {
                        elem.reserve(numSamp);
                    }
                }
                ...
                for (size_t i = 0; i < numSamp; i++)
                {
                    ...
                    if (tsBytes == 8)
                    {
                        double ts;
                        file.read((char*)&ts, 8);
                        XDFdata.streams[index].time_stamps.push_back(ts);
                    }
                    else
                    {
                    ...
                    for (int v = 0; v < XDFdata.streams[index].info.channel_count; ++v)
                    {
                        ...
                        else
                        {
                            if (XDFdata.streams[index].info.channel_format.compare("float32") == 0)
                            {
                                ...
                                XDFdata.streams[index].time_series[v].push_back(data);
                            }
			...
			case 4: //read [ClockOffset] chunk
			{
                ...
                XDFdata.streams[index].clock_times.push_back(collectionTime);
                XDFdata.streams[index].clock_values.push_back(offsetValue);
			}
			break;
        ...
        for (size_t x = 0; x< XDFdata.streams.size(); x++)
        {
                     ...
                     XDFdata.streams[x].info.nominal_srate != 0)
            {
                ...
                for (size_t w = 0; w < XDFdata.streams[x].time_series.size(); w++)
                {
                    ...
                    XDFdata.streams[x].time_series[w].reserve(OUT_BUF_SIZE);
